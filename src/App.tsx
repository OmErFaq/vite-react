...
interface Guest {
  ...
  dynamic?: boolean;
  docId?: string; // Added to track Firebase document IDs for editing
}
...
function AdminPanel({ onClose, dynamicGuests, editingGuest }: { onClose: () => void; dynamicGuests: Guest[], editingGuest: Guest | null }) {
  ...
  const handleSave = async () => {
    ...
    // Determine the ID: 
    // - If editing a dynamic guest, keep its existing Firebase docId
    // - If overriding a BASE_GUEST, use their id (e.g., '001') as the Firebase doc ID so we can match it later
    // - If brand new, generate a new id
    ...
    try {
      await setDoc(doc(GUESTS_COL, firebaseDocId), {
        id: guestId, // Maintain original ID in the data
        name: form.name.trim(),
      ...
      }, { merge: true }); // Merge ensures we update rather than overwrite entirely if fields are missing

      setSaved(true);
      setTimeout(() => {
        setSaved(false);
        if (isEditing) onClose(); // Auto-close after editing
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.85)",
      ...
      <div style={{background:GLASS,backdropFilter:"blur(24px)",borderRadius:22,
        ...
        {/* Recently added (only show if not editing) */}
...
export default function IftarCheckin() {
  ...
  useEffect(() => {
    const loadDyn = async () => {
      try {
        ...
        // Include the Firebase document ID so we know exactly which document to update
  ...
  // If a base guest was edited and saved to Firebase, it will have the same 'id'.
  // We filter out base guests that have an overriding match in `dynGuests`.
