...
export default function IftarCheckin() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      CHECKIN_DOC,
      (snap: DocumentData) => {
        setCheckins(snap.exists() ? snap.data() as Record<string,string> : {});
        setDbStatus("ok");
      },
      (_err: Error) => {
        setDbStatus("error");
  ...
  const search = () => {
    ...
    setResults(GUESTS.filter((g: Guest) => norm(g.name).includes(q) || norm(g.role).includes(q)));
    setSearched(true);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:460,margin:"0 auto",padding:"0 16px 60px"}}>
        ...
        {dbStatus==="connecting" && (
          <div style={{background:"rgba(26,184,216,0.15)",borderRadius:10,padding:"10px 16px",margin:"16px 0",fontSize:13,textAlign:"center",color:TEAL2,border:`1px solid ${BORDER}`}}>
            Connecting…
          </div>
        )}

        <div style={{textAlign:"center",paddingTop:30,marginBottom:18}}>
          <div style={{display:"inline-block",background:"#fff",borderRadius:12,padding:"8px 20px",boxShadow:`0 0 0 1px ${BORDER}, 0 6px 28px rgba(0,0,0,0.55)`}}>
            <img src={`data:image/png;base64,${LOGO_B64}`} alt="Smartplus" style={{height:32,objectFit:"contain",display:"block"}}/>
          </div>
        </div>

        <div style={{background:"rgba(8,44,72,0.65)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderRadius:22,border:"1px solid rgba(96,216,248,0.18)",padding:"32px 24px 28px",boxShadow:"0 8px 48px rgba(0,0,0,0.5),inset 0 1px 0 rgba(96,216,248,0.1)",marginBottom:18,textAlign:"center"}}>
          <div style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:30,color:TEAL2,letterSpacing:2,marginBottom:2,textShadow:`0 0 20px ${TEAL},0 0 40px rgba(26,184,216,0.4)`}}>Ramadan</div>
          <div style={{fontSize:80,fontWeight:900,letterSpacing:6,lineHeight:1,color:"#fff",textShadow:"0 2px 40px rgba(96,216,248,0.15)",marginBottom:10}}>IFTAR</div>
        ...
        <div style={{background:GLASS,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderRadius:18,border:`1px solid ${BORDER}`,padding:"20px 18px",boxShadow:"0 4px 30px rgba(0,0,0,0.3)",marginBottom:14}}>
          <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:TEAL2,marginBottom:12,opacity:0.85}}>Find Your Seat · ابحث عن مقعدك</div>
          <div style={{display:"flex",gap:10}}>
            <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&search()} dir="auto" placeholder="Your name · اسمك"
          ...
          return (
            <div key={guest.id} style={{background:checked?"rgba(6,52,36,0.7)":GLASS,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderRadius:18,marginBottom:14,border:`1px solid ${checked?"rgba(30,200,100,0.35)":tentative?"rgba(240,200,60,0.28)":BORDER}`,padding:"20px 18px",boxShadow:"0 4px 28px rgba(0,0,0,0.3)",transition:"all 0.3s"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                ...
                <div style={{fontSize:9,letterSpacing:1.5,textTransform:"uppercase",fontWeight:700,padding:"4px 11px",borderRadius:20,whiteSpace:"nowrap",flexShrink:0,marginLeft:10,background:tentative?"rgba(240,200,60,0.1)":"rgba(26,184,216,0.1)",border:`1px solid ${tentative?"rgba(240,200,60,0.4)":"rgba(96,216,248,0.35)"}`,color:tentative?"#f0c84a":TEAL2}}>
              ...
              {checked && time && (
                <div style={{fontSize:11,color:"rgba(50,200,110,0.6)",textAlign:"center",marginTop:5}}>Checked in at {time}</div>
              )}
            </div>
          );
        })}
