...
//  2. Create a project called "iftar-checkin"
...
export default function IftarCheckin() {
  ...
  // ── Real-time listener — all devices stay in sync instantly ────────────────
  useEffect(() => {
  ...
  const search = () => {
    ...
    setResults(GUESTS.filter(g => norm(g.name).includes(q) || norm(g.role).includes(q)));
    setSearched(true);
  };

  const checkIn = async (guest) => {
    try {
      const current = (await getDoc(CHECKIN_DOC)).data() || {};
      await setDoc(CHECKIN_DOC, {
        ...current,
        [guest.id]: new Date().toISOString(),
      });
    } catch (e) {
      alert("Check-in failed — check internet connection.");
    }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:460,margin:"0 auto",padding:"0 16px 60px"}}>
        ...
        {/* ── DB Status banner ── */}
        {dbStatus === "error" && (
          <div style={{background:"rgba(180,40,40,0.9)",borderRadius:10,padding:"10px 16px",margin:"16px 0",
            fontSize:13,textAlign:"center",color:"#fff"}}>
            ⚠️ Cannot connect to database — check internet connection
          </div>
        )}
        {dbStatus === "connecting" && (
          <div style={{background:"rgba(26,184,216,0.15)",borderRadius:10,padding:"10px 16px",margin:"16px 0",
            fontSize:13,textAlign:"center",color:TEAL2,border:`1px solid ${BORDER}`}}>
            Connecting…
          </div>
        )}

        {/* ── Logo ── */}
        <div style={{textAlign:"center",paddingTop:30,marginBottom:18}}>
          <div style={{display:"inline-block",background:"#fff",borderRadius:12,padding:"8px 20px",
            boxShadow:`0 0 0 1px ${BORDER}, 0 6px 28px rgba(0,0,0,0.55)`}}>
            <img src={`data:image/png;base64,${LOGO_B64}`} alt="Smartplus"
        ...
        {/* ── Hero card ── */}
        <div style={{
          background:"rgba(8,44,72,0.65)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",
          borderRadius:22,border:"1px solid rgba(96,216,248,0.18)",padding:"32px 24px 28px",
          boxShadow:"0 8px 48px rgba(0,0,0,0.5),inset 0 1px 0 rgba(96,216,248,0.1)",
          marginBottom:18,textAlign:"center",
        }}>
          <div style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:30,color:TEAL2,
            letterSpacing:2,marginBottom:2,textShadow:`0 0 20px ${TEAL},0 0 40px rgba(26,184,216,0.4)`}}>
            Ramadan
          </div>
          <div style={{fontSize:80,fontWeight:900,letterSpacing:6,lineHeight:1,color:"#fff",
            textShadow:"0 2px 40px rgba(96,216,248,0.15)",marginBottom:10}}>
            IFTAR
          </div>
          <div style={{display:"inline-block",background:TEAL,color:"#031c2e",fontSize:18,fontWeight:800,
            letterSpacing:3,padding:"5px 22px",borderRadius:8,marginBottom:24,
            boxShadow:`0 0 20px rgba(26,184,216,0.5),0 0 40px rgba(26,184,216,0.2)`}}>
            2026
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",gap:10}}>
              {[["📅","March 8th"],["🕐","05:00PM – 08:00PM"]].map(([icon,text])=>(
                <div key={text} style={{flex:1,background:"rgba(2,18,32,0.7)",borderRadius:10,
            ...
            <div style={{background:"rgba(2,18,32,0.7)",borderRadius:10,padding:"11px 16px",
              border:"1px solid rgba(255,255,255,0.1)",display:"flex",alignItems:"center",gap:8,fontSize:13}}>
              <span style={{fontSize:15}}>📍</span>
              <span><strong>Hyatt Regency Hotel</strong>
                <span style={{color:TEAL2}}> — Riyadh Olaya</span>
              </span>
            </div>
          </div>
        </div>
        ...
        {/* ── Search ── */}
        <div style={{background:GLASS,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
          borderRadius:18,border:`1px solid ${BORDER}`,padding:"20px 18px",
          boxShadow:"0 4px 30px rgba(0,0,0,0.3)",marginBottom:14}}>
          <div style={{fontSize:10,letterSpacing:2.5,textTransform:"uppercase",color:TEAL2,marginBottom:12,opacity:0.85}}>
            Find Your Seat · ابحث عن مقعدك
          </div>
          <div style={{display:"flex",gap:10}}>
            <input value={query} onChange={e=>setQuery(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&search()} dir="auto"
              placeholder="Your name · اسمك"
              style={{flex:1,background:"rgba(2,14,26,0.6)",border:`1px solid rgba(96,216,248,0.22)`,
                borderRadius:10,padding:"12px 14px",fontSize:14,color:"#d8f0f8",
                outline:"none",fontFamily:"inherit"}}/>
            <button onClick={search} style={{
              background:`linear-gradient(135deg,${TEAL},${TEAL2})`,border:"none",
              borderRadius:10,padding:"12px 16px",cursor:"pointer",color:"#031c2e",
              display:"flex",alignItems:"center",boxShadow:`0 0 16px rgba(26,184,216,0.35)`}}>
              <SearchIcon/>
            </button>
          </div>
          <div style={{fontSize:11,color:"rgba(216,240,248,0.35)",marginTop:8,textAlign:"center"}}>
        ...
        {searched && results.length===0 && (
          <div style={{background:GLASS,backdropFilter:"blur(16px)",borderRadius:18,
            border:`1px solid ${BORDER}`,padding:"32px 20px",textAlign:"center"}}>
            <div style={{fontSize:32,marginBottom:10}}>🔍</div>
            <div style={{color:"rgba(216,240,248,0.55)",fontSize:14}}>Name not found. Please see the welcome team.</div>
            <div style={{direction:"rtl",marginTop:6,fontSize:13,color:"rgba(216,240,248,0.4)"}}>
              لم يتم العثور على اسمك. تواصل مع فريق الاستقبال
            </div>
          </div>
        )}
        ...
        {results.map(guest => {
          ...
          const time      = checkins[guest.id]
            ? new Date(checkins[guest.id]).toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}) : null;

          return (
            <div key={guest.id} style={{
              background:checked?"rgba(6,52,36,0.7)":GLASS,
              backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",
              borderRadius:18,marginBottom:14,
              border:`1px solid ${checked?"rgba(30,200,100,0.35)":tentative?"rgba(240,200,60,0.28)":BORDER}`,
              padding:"20px 18px",boxShadow:"0 4px 28px rgba(0,0,0,0.3)",transition:"all 0.3s",
            }}>
              {/* Name + status */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                <div>
                  <div style={{fontSize:17,fontWeight:700,color:"#fff",lineHeight:1.3}}>{guest.name}</div>
                  <div style={{fontSize:12,color:"rgba(216,240,248,0.45)",marginTop:3}}>{guest.role}</div>
                </div>
                <div style={{
                  fontSize:9,letterSpacing:1.5,textTransform:"uppercase",fontWeight:700,
                  padding:"4px 11px",borderRadius:20,whiteSpace:"nowrap",flexShrink:0,marginLeft:10,
                  background:tentative?"rgba(240,200,60,0.1)":"rgba(26,184,216,0.1)",
                  border:`1px solid ${tentative?"rgba(240,200,60,0.4)":"rgba(96,216,248,0.35)"}`,
                  color:tentative?"#f0c84a":TEAL2,
                }}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
                {[
                  {label:"Table",  val:guest.table,  color:"#f0c84a", large:true},
                  {label:"Side",   val:guest.side==="Left"?"◀ Left":"Right ▶", color:TEAL2, large:false},
                  {label:"Door",   val:guest.entrance, color:TEAL, circle:true},
                ].map(({label,val,color,large,circle})=>(
                  <div key={label} style={{background:"rgba(2,14,26,0.6)",borderRadius:10,padding:"10px 8px",
                    textAlign:"center",border:"1px solid rgba(255,255,255,0.06)"}}>
                    <div style={{fontSize:8,letterSpacing:1.5,textTransform:"uppercase",
                      color:"rgba(216,240,248,0.3)",marginBottom:5}}>{label}</div>
                    {circle
                      ? <div style={{width:28,height:28,borderRadius:"50%",margin:"0 auto",
                          background:`linear-gradient(135deg,${TEAL},${TEAL2})`,
                          display:"flex",alignItems:"center",justifyContent:"center",
                          fontSize:15,fontWeight:900,color:"#031c2e",
                          boxShadow:`0 0 10px rgba(26,184,216,0.45)`}}>{val}</div>
              ...
              <div style={{fontSize:11,color:"rgba(216,240,248,0.38)",textAlign:"center",marginBottom:12}}>
                {guest.entrance==="A"
                  ? "Use Entrance A · left doors · المدخل A"
                  : "Use Entrance B · right doors · المدخل B"}
              </div>

              <button onClick={()=>!checked&&checkIn(guest)} disabled={checked} style={{
                width:"100%",padding:"12px",borderRadius:10,border:"none",
                cursor:checked?"default":"pointer",fontSize:12,fontWeight:700,
                ...
                background:checked?"rgba(30,180,90,0.14)":`linear-gradient(135deg,${TEAL},${TEAL2})`,
                color:checked?"rgb(50,200,110)":"#031c2e",
                boxShadow:checked?"none":`0 0 18px rgba(26,184,216,0.3)`,
              }}>
                {checked?<><CheckIcon/> Checked In · تم التسجيل</>:"Tap to Check In · اضغط للتسجيل"}
              </button>
              {checked && time && (
                <div style={{fontSize:11,color:"rgba(50,200,110,0.6)",textAlign:"center",marginTop:5}}>
                  Checked in at {time}
                </div>
              )}
            </div>
        ...
        <div style={{textAlign:"center",marginTop:44,opacity:0.2,fontSize:11,letterSpacing:2}}>
