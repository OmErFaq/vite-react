...
// ── ADMIN PANEL ──────────────────────────────────────────────────────────────
function AdminPanel({ onClose, dynamicGuests }: { onClose: () => void; dynamicGuests: Guest[] }) {
  const TEAL = "#1ab8d8", TEAL2 = "#60d8f8";
  const GLASS = "rgba(8,44,72,0.95)";
  const [form, setForm] = useState({ name:"", role:"", table:"12", status:"confirmed" });
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState("");

  const tableNum = parseInt(form.table) || 12;
  const entrance = tableNum <= 6 ? "A" : "B";
  const side     = tableNum <= 6 ? "Left" : "Right";

  const handleAdd = async () => {
    if (!form.name.trim()) { setError("Name is required"); return; }
    setSaving(true); setError("");
    try {
      await addDoc(GUESTS_COL, {
        id: `dyn_${Date.now()}`,
        name: form.name.trim(),
      ...
      setForm({ name:"", role:"", table:"12", status:"confirmed" });
      setSaved(true);
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.85)",
      ...
      <div style={{background:GLASS,backdropFilter:"blur(24px)",borderRadius:22,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div>
            <div style={{fontSize:18,fontWeight:700,color:"#fff"}}>➕ Add Guest</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:TEAL2,marginTop:5}}>
              → Entrance <strong>{entrance}</strong> &nbsp;·&nbsp; {side} side
            </div>
          </div>
          <div>
          ...
          {saved && <div style={{fontSize:12,color:"#50c878",textAlign:"center"}}>✓ Guest added successfully!</div>}
...
export default function IftarCheckin() {
  ...
  // Real-time dynamic guests
  useEffect(() => {
    const loadDyn = async () => {
      try {
        const snap = await getDocs(GUESTS_COL);
        const list: Guest[] = snap.docs.map(d => d.data() as Guest);
        setDynGuests(list);
      } catch { /* silent */ }
    };
    loadDyn();
  ...
  const allGuests = [...BASE_GUESTS, ...dynGuests];
  ...
  const handleAdminLogin = () => {
    if (adminInput === ADMIN_PASSWORD) {
      setAdminUnlocked(true); setAdminError(false); setShowAdmin(true);
    } else {
      setAdminError(true);
    }
  };
  ...
  return (
    <div style={{minHeight:"100vh",width:"100%",
      ...
      {showAdmin && adminUnlocked && (
        <AdminPanel onClose={()=>setShowAdmin(false)} dynamicGuests={dynGuests}/>
      )}
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:460,margin:"0 auto",padding:"0 16px 60px"}}>
        ...
        <div style={{textAlign:"center",paddingTop:30,marginBottom:18,position:"relative"}}>
          ...
          {/* Subtle admin button */}
          <button onClick={()=>adminUnlocked?setShowAdmin(true):setShowAdmin(s=>!s)}
            ...
            <div key={guest.id} style={{background:checked?"rgba(6,52,36,0.7)":GLASS,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderRadius:18,marginBottom:14,border:`1px solid ${checked?"rgba(30,200,100,0.35)":tentative?"rgba(240,200,60,0.28)":BORDER}`,padding:"20px 18px",boxShadow:"0 4px 28px rgba(0,0,0,0.3)",transition:"all 0.3s"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:"rgba(216,240,248,0.45)",marginTop:3}}>{guest.role}{guest.dynamic&&<span style={{marginLeft:8,fontSize:10,color:TEAL2,background:"rgba(26,184,216,0.12)",padding:"1px 6px",borderRadius:10}}>new</span>}</div>
                </div>
